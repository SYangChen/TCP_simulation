...
packet UDP_receive() {
	...
	if ( packetReceive.packetHeader.checkSum == 87 ) 
		printf( "\033[0;31m*************** Packet(CheckSum) error Detect ***************\033[0m\n" );
	else if (recvlen > 0) {
		// buf[recvlen] = 0;
...
void ReceiveFile() {
	...
	for ( v = 0 ; v < 5 ; ++v ) {
		...
		while ( 1 ) {
			...
			printf( "****CalCheckSum %d ****************\n", CaculateCheckSum( receivePacket ) ) ;
			printf( "****RecCheckSum %d ****************\n", receivePacket.packetHeader.checkSum ) ;
			checks = CaculateCheckSum( receivePacket ) ;
			if ( checks + receivePacket.packetHeader.checkSum != 65535 )
			 	return ;
			WriteData( receivePacket.data, recvlen-sizeof( receivePacket.packetHeader )-8 ) ;
			sendPacket.packetHeader.seqNum = ++sequence ;
			sendPacket.packetHeader.ackNum = receivePacket.packetHeader.seqNum+DEFAULT_MSS ;
