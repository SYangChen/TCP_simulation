...
void SlowStart() {
	...
	for ( i = 0 ; i < cwnd ; ++i ) {
		...
		packetSend.packetHeader.seqNum = sequence ;
		++count ;
		if ( sequence == 4097 && !error8193 ) {
			error8193 = 1 ;
			printf( "Create a packet loss at byte 4097\n" ) ;
		}
		else {
	...
	for ( i = 0 ; i < cwnd && i < count ; ++i ) {
		...
		if (recvlen > 0) {
			...
			if ( dupACKcount == 3 ) {
				...
				cwnd = ssthresh/DEFAULT_MSS+3 ;	// 1 MSS
				congestionCtrl_state = FAST_RECOVERY ;
				// dupACKcount = 1 ;
				sequence = dupACKnum ;
				// dupACKnum = 0 ;
